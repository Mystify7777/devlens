...
describe("createInspector", () => {
  ...
  it("renders the empty state immediately on creation", () => {
    const inspector = createInspector();
    expect(inspector.element.querySelector("[data-devlens-inspector-empty]")).not.toBeNull();
  });
  ...
  it("renders event detail when given an event", () => {
    ...
    expect(inspector.element.querySelector("[data-devlens-inspector-title]")?.textContent).toBe(
      "Selected Event"
    );
  });
  ...
  it("clears the empty state once an event is rendered", () => {
    ...
    expect(inspector.element.querySelector("[data-devlens-inspector-empty]")).toBeNull();
  });
  ...
  it("returns to the empty state when rendered with null", () => {
    ...
    expect(inspector.element.querySelector("[data-devlens-inspector-empty]")).not.toBeNull();
    expect(inspector.element.querySelector("[data-devlens-inspector-title]")).toBeNull();
  });

  it("renders severity, title, and message for a minimal event", () => {
    const inspector = createInspector();
    inspector.render(makeEvent({ severity: "warn", title: "A Title", message: "A message" }), "");

    expect(inspector.element.querySelector("[data-devlens-inspector-severity]")?.textContent).toBe(
      "WARN"
    );
    expect(inspector.element.querySelector("[data-devlens-inspector-title]")?.textContent).toBe(
      "A Title"
    );
    expect(inspector.element.querySelector("[data-devlens-inspector-message]")?.textContent).toBe(
      "A message"
    );
  });
  ...
  it("omits the stack section entirely when the event has no stack", () => {
    ...
    expect(inspector.element.querySelector("[data-devlens-inspector-stack]")).toBeNull();
  });
  ...
  it("renders the stack section when the event has one", () => {
    ...
    expect(inspector.element.querySelector("[data-devlens-inspector-stack]")?.textContent).toBe(
      "Error: boom\n at line 1"
    );
  });
  ...
  it("omits the metadata section when metadata is absent", () => {
    ...
    expect(inspector.element.querySelector("[data-devlens-inspector-metadata]")).toBeNull();
  });
  ...
  it("renders each metadata entry generically, with no hardcoded field names", () => {
    const inspector = createInspector();
    inspector.render(makeEvent({ metadata: { statusCode: 404, url: "/api/widgets" } }), "");

    const keys = Array.from(
      inspector.element.querySelectorAll("[data-devlens-inspector-metadata-key]")
    ).map((el) => el.textContent);
    const values = Array.from(
      inspector.element.querySelectorAll("[data-devlens-inspector-metadata-value]")
    ).map((el) => el.textContent);
  ...
  it("stringifies non-string metadata values without throwing", () => {
    const inspector = createInspector();
    inspector.render(makeEvent({ metadata: { nested: { retries: 3 }, ok: false } }), "");
  ...
  it("omits the context section when context is absent", () => {
    ...
    expect(inspector.element.querySelector("[data-devlens-inspector-context]")).toBeNull();
  });
  ...
  it("renders context entries the same generic way as metadata", () => {
    ...
    expect(
      inspector.element.querySelector("[data-devlens-inspector-context-key]")?.textContent
    ).toBe("userAgent");
    expect(
      inspector.element.querySelector("[data-devlens-inspector-context-value]")?.textContent
    ).toBe("test-agent");
  });
  ...
  it("omits the tags section when tags is absent", () => {
    ...
    expect(inspector.element.querySelector("[data-devlens-inspector-tags]")).toBeNull();
  });
  ...
  it("renders one tag element per tag, in order", () => {
    ...
    const tags = Array.from(inspector.element.querySelectorAll("[data-devlens-inspector-tag]")).map(
      (el) => el.textContent
    );
  ...
  it("replaces prior detail rather than appending when rendering a new event", () => {
    ...
    const titles = inspector.element.querySelectorAll("[data-devlens-inspector-title]");
  ...
  it("never uses innerHTML — a title containing markup is not parsed as an element", () => {
    ...
    const titleEl = inspector.element.querySelector("[data-devlens-inspector-title]");
  ...
  it("never uses innerHTML for metadata values either", () => {
    const inspector = createInspector();
    inspector.render(makeEvent({ metadata: { note: "<script>alert(1)</script>" } }), "");

    const valueEl = inspector.element.querySelector("[data-devlens-inspector-metadata-value]");
  ...
  describe("search highlighting", () => {
    ...
    it("highlights a matching substring in the message", () => {
      const inspector = createInspector();
      inspector.render(makeEvent({ message: "connection timed out" }), "timed out");

      const messageEl = inspector.element.querySelector("[data-devlens-inspector-message]");
    ...
    it("highlights a matching substring in the stack", () => {
      const inspector = createInspector();
      inspector.render(makeEvent({ stack: "at handleClick (app.js:42)" }), "handleClick");

      const stackEl = inspector.element.querySelector("[data-devlens-inspector-stack]");
      expect(stackEl?.querySelector("[data-devlens-match]")?.textContent).toBe("handleClick");
    });

    it("highlights a matching tag", () => {
      const inspector = createInspector();
      inspector.render(makeEvent({ tags: ["network", "retryable"] }), "retryable");

      const tags = inspector.element.querySelectorAll("[data-devlens-inspector-tag]");
      expect(tags[0].querySelector("[data-devlens-match]")).toBeNull();
      expect(tags[1].querySelector("[data-devlens-match]")?.textContent).toBe("retryable");
    });
    ...
    it("does not highlight anything when the query is empty", () => {
      ...
      expect(inspector.element.querySelector("[data-devlens-match]")).toBeNull();
    });
    ...
    it("never highlights severity — severity is not a searchable field", () => {
      ...
      const severityEl = inspector.element.querySelector("[data-devlens-inspector-severity]");
    ...
    it("never highlights metadata values — metadata is not a searchable field", () => {
      const inspector = createInspector();
      inspector.render(makeEvent({ metadata: { note: "timeout occurred" } }), "timeout");
    ...
    it("never highlights context values — context is not a searchable field", () => {
      const inspector = createInspector();
      inspector.render(makeEvent({ context: { note: "timeout occurred" } }), "timeout");

      const valueEl = inspector.element.querySelector("[data-devlens-inspector-context-value]");
      expect(valueEl?.querySelector("[data-devlens-match]")).toBeNull();
    });
  });
});
